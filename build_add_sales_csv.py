"""Transform the level_1 sales history into per-reseller CSVs for `testdata add-sales`.
...
For each reseller (Heybox, Sonkwo) it writes
``data/build_add_sales_csv/<reseller>.csv`` with the columns add-sales expects:
...
def main() -> int:
    ...
    for reseller in RESELLERS:
        ...
        path = args.output_dir / f"{reseller.lower()}.csv"
        ...
        print(f"Wrote {path}: {len(sub)} rows, {sub['cref'].nunique()} SKUs, "
              f"{int(sub['qty'].sum())} units", file=sys.stderr)
