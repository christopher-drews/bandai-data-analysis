...
the supplier catalog's ``paPaxCode`` field). ``start_month``/``end_month``
expand to first-of-month / last-of-month dates.
...
def fetch_pax_to_sku(session: requests.Session, host: str, org_id: str, headers: dict) -> dict[str, str]:
    ...
    while True:
        ...
        for item in items:
            ...
            if not pax or not sku:
                continue
            if pax in mapping and mapping[pax] != sku:
    ...
    for pax, skus in duplicates.items():
        print(f"  warn: paxCode {pax!r} maps to multiple SKUs: {[mapping[pax], *skus]}", file=sys.stderr)
    return mapping
...
def main() -> int:
    ...
    pax_to_sku = fetch_pax_to_sku(session, args.host, args.org_id, headers)
    print(f"  {len(pax_to_sku)} SKUs with paPaxCode", file=sys.stderr)
    ...
    for row in rows:
        ...
        if not sku_id:
            ...
            skipped += 1
            continue
        if not (srp_raw and currency and start_month and end_month):
            print(f"  skip {product!r}: missing SRP/currency/months", file=sys.stderr)
            skipped += 1
            continue

        start_date, end_date = month_bounds(start_month, end_month)
        body = {
        ...
        prefix = f"{pax} ({sku_id}) {currency}={srp_raw} {start_date}->{end_date}"
        if args.dry_run:
